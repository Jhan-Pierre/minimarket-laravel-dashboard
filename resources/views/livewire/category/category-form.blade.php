<div>
            ...
            <tbody>
                @foreach ($categories as $category)
                    <tr>
                        ...
                        <td class="px-6 py-3">{{ $category->nombre }}</td>

                        <td class="px-6 py-3 flex items-center justify-end">
                            @can('admin.category.delete')
                                <a wire:click="delete({{ $category->id }}, '{{ $category->name }}')" class="rounded-lg block py-2 px-4 text-sm text-gray-700 hover:bg-red-400 dark:hover:bg-red-600 dark:text-gray-200 dark:hover:text-white">
                            ...
                            @can('admin.category.edit')
                                <a wire:click="edit({{ $category->id }})" class="rounded-lg block py-2 px-4 hover:bg-green-400 dark:hover:bg-green-600 dark:hover:text-white">
                                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        ...
        <x-modal-edit-flow>
            ...
            <form wire:submit="save">
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <x-label-flow for="categoryCreate.nombre">Nombre</x-label-flow>
                        <x-input-flow type="text" id="categoryCreate.nombre" wire:model="categoryCreate.nombre"/>
                        <x-input-error-flow for='categoryCreate.nombre' />
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <button type="button" data-modal-toggle="2" wire:click="closeCreate" class="text-red-600 inline-flex items-center hover:text-white border border-red-600 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                        Cancelar
                    </button>
                </div>
            </form>
    ...
    @if ($categoryEdit->open)
        <x-modal-edit-flow>
            <x-slot name="header">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    Editar Categoria
                </h3>
                <button wire:click="$set('categoryEdit.open', false)" data-modal-toggle="2" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            <form wire:submit="update">
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <x-label-flow for="nombre">Nombre</x-label-flow>
                        <x-input-flow type="text" name="nombre" wire:model="categoryEdit.nombre"/>
                        <x-input-error-flow for='categoryEdit.nombre' />
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <button type="button" wire:click="$set('categoryEdit.open', false)" class="text-red-600 inline-flex items-center hover:text-white border border-red-600 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                        Cancelar
                    </button>
                </div>
            </form>
    ...
    @if ($openDelete)
        <x-modal-delete-flow maxWidth="lg">
            ...
            <p class="mb-4 text-gray-500 dark:text-gray-300">¿Deseas eliminar la categoría "{{ $categoryDeleteName }}" con ID "{{ $categoryDeleteId }}"? </p>

            <x-slot name="footer">
                <button wire:click="closeModalAndResetButton" data-modal-toggle="deleteModal" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                    No, cancelar
                </button>

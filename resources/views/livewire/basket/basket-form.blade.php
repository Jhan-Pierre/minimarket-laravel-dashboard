<div>
    ...
    <div class="relative sm:rounded-lg">
        <div class="flex flex-col items-center p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
            ...
            <x-input-error-flow for='basketCreate.codbarras'/>
        </div>
    </div>
                    ...
                    @foreach ($baskets as $basket)
                        @if ($basket->cantidad > 0)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" wire:key="{{ $basket->id }}-product">
                                ...
                                <td class="px-6 py-3">
                                    <button {{ $basket->cantidad <= 1 ? 'disabled' : '' }} wire:click="decrementQuantity({{ $basket->product_id }})" type="button" id="decrement-button-{{ $basket->id }}" class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-300 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700">
                                        <svg class="h-2.5 w-2.5 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                                          <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 1h16" />
                                        </svg>
                                    </button>
                                    <input value="{{ $basket->cantidad }}" disabled type="text" id="counter-input" data-input-counter class="w-10 shrink-0 border-0 bg-transparent text-center text-sm font-medium text-gray-900 focus:outline-none focus:ring-0 dark:text-white" placeholder="" required />
                                    <button wire:click="incrementQuantity({{ $basket->product_id }})" type="button" id="increment-button-{{ $basket->id }}" class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-300 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700">
